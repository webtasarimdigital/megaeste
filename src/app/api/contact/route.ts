...
export async function POST(request: Request) {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      secure: true, 
      auth: {
        user: process.env.SMTP_USER || 'info@megaeste.com',
        pass: process.env.SMTP_PASS || 'Asdasd456+',
      },
      tls: {
    ...
    const userEmail = process.env.SMTP_USER || 'info@megaeste.com';

    const mailOptions = {
      from: `"Megaeste Form" <${userEmail}>`,
      to: userEmail,
      replyTo: email, 
